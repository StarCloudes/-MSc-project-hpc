...
std::vector<double> simulate_monte_carlo(const std::vector<Debtor>& portfolio,
    ...
    for (int i = 0; i < num_paths; ++i) {
        ...
        // Generate stratified Gaussian sample with external RNG
        std::vector<double> z = generate_stratified_correlated_gaussian(L, N, i, num_paths, gen);
        ...
        if ((i + 1) % report_interval == 0) {
            std::cout << "[Rank " << mpi_rank << "] [Progress] Simulated "
                      << (i + 1) << " / " << num_paths
